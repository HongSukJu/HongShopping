...
class CartItemListViewAdapter(private val items: ArrayList<CartItem>, val onItemClick: (CartItem) -> Unit) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val priceCommaAndQuantity = NumberFormat.getNumberInstance(Locale.KOREA).format(item.price) + "원 X " + item.quantity
        holder.priceAndQuantity.text = priceCommaAndQuantity
        val totalPriceComma = NumberFormat.getNumberInstance(Locale.KOREA).format(item.price * item.quantity) + "원"
        holder.totalPrice.text = totalPriceComma
    }
}